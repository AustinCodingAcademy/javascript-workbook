...
//  Scope defines the availability of an item.  Scope can be global, meaning the item can be accessed from anywhere in the program.
//  Or the scope can be limited, if the item is instantiated within a function.
...
//
// Your Answer Goes Here:
// The ternary operator is '?'.  It provides a shorthand version of an if statement.  Instead of writing a full-syntax if statement,
// one can simply write the conditional expression, followed by '?'.  Then provide the actions for truthy, followed by the action for falsey.
