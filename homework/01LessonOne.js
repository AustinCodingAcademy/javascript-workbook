...
//Null has an actual value of null, sort of a placeholder. Undefined has not value at all.
...
//And checks to see if each individual element you are comparing is true. The or operator allows at least one element to be true to make the entire expression true. This can be especially helpful when determining what should happen based on various potential outcomes.
// ****
...
//
// Your Answer Goes Here:
// An expression is "truthy" if it's emotionally positive. Examples include strings, numbers, etc. An expression is "falsey" if it's emotionally negative. Examples include undefined, null, empty string, 0.
// ****
...
//
// Your Answer Goes Here:
// == compares the two values on either side of the ==, but it is not strict. That means that it tries to convert one side of the equation to the same type as the other side before comparing the values. So, if you have a number and a string, == could compare both values as strings, or both values as numbers. === compares the exact values (including types) on either side of the equation. They have to be the exact same to be true, strictly equal.
//
// What’s the difference between != and !==?
//
// Your Answer Goes Here:
// Similar to the explanation above, != will compare two values to see if they are not equal. It will try to compare the values on either side of the operation as the same type. !== will compare the two exact values/types to see if they are equal.
// ****
