...
//The and operator dictates that both conditions must be true.  You would use this to apply conditional logic to an expression where both conditions must be satisfied. The "or" operator says that any one of multiple conditions can be true in order to be considered satisfied.  You could apply this to an expression where multiple possible conditions can be true in order to satisy the expression.
// ****
...
//
// Your Answer Goes Here:
// Sometimes it's necessary to test a condition for a positive or negative value.  The positive value or "true" is considered truthey.  A negative value or "false" is considered falsey.
// ****
...
//
// Your Answer Goes Here:
// These are operators used to perform a comparison of two values. == is an abstract comparison.  Javascript could possibly resolve two data types that are similar.  For example, 4 and '4' are technically two different data types.  An abstract comparison of 4 == '4' would come back true because javascript recognized that '4' is actually a number data type (even though it's in quotes) and completes the comparison.  === is a strict comparison which dictates that both the values and data types must be the same in order to return true.  Using the same example, 4 === '4' would return false because '4' is technically a string value and not comparable to a number data type.
//
// What’s the difference between != and !==?
//
// Your Answer Goes Here:
// Similar to ==, != is the abstract comparison of values.  Javascript can automatically resolve data types even though they may not be exact.  !== is the stricter interpretation of the comparison such that both the value and data type must be an exact match.
// ****
