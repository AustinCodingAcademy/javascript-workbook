...
$(document).on('ready', function() {
  ...
  var problemNine = $('#insert-text');
  problemNine.text('Text Has Been Inserted');

  // Problem 10:
  // select the element with id="add-attribute"
  // add the attribute data-color="red" to that element
  var problemTen = $('#add-attribute');
  problemTen.attr('data-color', 'red');
  ...
  var problemEleven = $('#data-element');
  window.dataIdValue = problemEleven.data('id');

  // Problem 12:
  // select the element with id="data-element"
  // and set its text (.text()) to the value of the data-content attribute
  var problemTwelve = $('#data-element');
  problemTwelve.text(problemTwelve.data('content'));
});
