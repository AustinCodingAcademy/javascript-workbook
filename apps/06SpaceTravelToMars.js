...
function CrewMember (name, job, specialSkill) {
  ...
  this.enterShip = function (ship) {
    this.ship=ship;
    ship.crew.push(this)
    console.log(this.name + ' joined the ' + this.ship.name + ' ship.')
  }
}
...
function Ship(name, type, ability) {
  ...
  this.missionStatement = function () {
    ...
    for (i=0; i<this.crew.length; i++) {
      var this_crew_member = this.crew[i];
      var this_crew_job = this_crew_member.job;
      console.log(this_crew_job + ' is the crew members job.')
      if (jobTypes[this_crew_job] === this.type) {
        return (this.ability)
      }
    }
