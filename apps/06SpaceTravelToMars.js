...
console.log(crewMember1.ship.name); //mav
...
function Ship(name, type, ability) {
    ...
    // following is a "method" (a method is an object with a function):
    this.missionstatement = function(){
      for (var i = 0; i < this.crew.length; i ++) {
        var thejob = this.crew[i].job;
        var typeofshiphecanfly = jobTypes[thejob];
        if (typeofshiphecanfly === this.type){
          return this.ability;
  }
      return "Can't perform a mission yet.";
    }
