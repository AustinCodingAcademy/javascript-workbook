...
function horizontalWin() {
    //so if we run a for loop then it can go through each row, then check for sequential elements that line up
    var boardIndex = 0;
...
function ticTacToe(row, column) {
    console.log("running TicTacToe, the row value is: " + row + " and the column value is " + column);
    //future addition create a validation check
    board[row][column] = playerTurn;
    //Check for a win
    checkForWin();
    //ternary operator which changes turns
