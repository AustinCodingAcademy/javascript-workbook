...
let nums = []
const stringsToNumbs = (numbers) => numbers.map(x => nums.push(Number(x)))
let a = stringsToNumbs(strNums)
console.log(a)
// With the same numbers, find the sum of the even values
const sumEvens = a.reduce((acc, val) => {
  if(val % 2 == 0)
  return acc + val;
  return acc
})
...
let track = []
weather.map(item => {
  if(!track.includes(item.weather_state_name)){
    track.push(item.weather_state_name)
  } 
} ) 
console.log(track) 
