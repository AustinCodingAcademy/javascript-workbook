...
const weatherStates = weather.map((weatherItem) => {
  return weatherItem.weather_state_name;
  })
  
  let uniqueItems = Array.from(new Set(weatherStates))
...
const idealTemp = weather.find((weatherItem) => {
  return weatherItem["min_temp"] === 15.915;
  });
  
  const perfectTemp = idealTemp.id;
