...
const atxIdx = nums.findIndex((currNumber, index, array) => {
  return currNumber + index === 512;
});
...
let uniqueWeatherStates = [];
weather.filter((w, i, array) => {
  if (uniqueWeatherStates.indexOf(w.weather_state_name) < 0) {
    uniqueWeatherStates.push(w.weather_state_name);
  }
});
const weatherStates = uniqueWeatherStates;
console.log(weatherStates);

//find the id of the object in weather that has a min_temp of 15.915

weather.forEach((w, i, arr) => {
  if (w.min_temp === 15.915) {
    console.log(w.id);
  }
  console.log(w.min_temp);
});

const idealTemp =
  weather[
    weather.findIndex(w => {
      return w.min_temp === 15.915;
    })
  ].id;
