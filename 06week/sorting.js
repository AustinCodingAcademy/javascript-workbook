...
console.log(Object.prototype.toString.call(stringsToNumbs[99]));

// With the same numbers, find the sum of the even values
const evens = stringsToNumbs.filter(num => num % 2 === 0);
const sumEvens = evens.reduce((acc, val) => acc + val);

console.log(sumEvens);

// Find the index of the first value when added to it's index = 512 (#ATX!!)
// How to use higher order functions for this?
for (let i = 0; i < stringsToNumbs.length; i++) {
  let total = i + stringsToNumbs[i];
  if (total === 512) {
    console.log(i, stringsToNumbs[i]);
    // return true;
  }
}
...
const idealTemp = weather.find(id => id.min_temp === 15.915);
