...
const stringsToNumbs = (numbers) => {
    ...
    for(i = 0; i < numbers.length; i++){
        let nums = parseInt(numbers[i], 10)
        newArr.push(nums)
    }
    return newArr;
}
...
const sumEvens = stringsToNumbs(strNums).filter(j => j % 2 === 0).reduce((total, amount) => total + amount)
...
const atxIdx = atxFinder(num);
console.log(`index: ${atxIdx}, value: ${num[atxIdx]}`);
...
for(i = 0; i < weather.length; i++){
    weatherStates.push(weather[i].weather_state_name)
}
for(let x = 0; x < weatherStates.length; x++){
    if(result.indexOf(weatherStates[x] === -1)){
        result.push(weatherStates[x]);
    }
};
console.log(result);

//find the id of the object in weather that has a min_temp of 15.915
function getTemp(weatherArr){
for(i = 0; i < weatherArr.length; i++){
if(weatherArr[i].min_temp == 15.915){
    return weatherArr[i].id
        }
    }
};
const idealTemp = getTemp(weather);
