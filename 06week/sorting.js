 ...
 const stringsToNums =numbers => numbers.map(number => Number(number))

 const digitsArr = stringsToNums(strNums);
 
 
 const sumEvens = arr => {
   const onlyEvens = num => num % 2 === 0;
   const evens = arr.filter(onlyEvens);
   const evenNums = (sum, currentValue) => sum + currentValue;
   return evens.reduce(evenNums)
 }
 sumEvens(digitsArr)
 ...
 const weatherStates = arr => {
  const weatherNames = arr.map(item => item.weather_state_name)
  const unique = (value, index, self) => {
    // console.log("value/index:", value, index)
    console.log("self:", self.indexOf(value), "index:",index)
    return self.indexOf(value)===index;
  }
  return weatherNames.filter(unique)
}
weatherStates(weather)
 ...
 const idealTemp = arr => {
  const correctItem = arr.find((item, index)=>{
