...
const stringsToNums = (numbers) => {
  let arrNums = [];
  numbers.forEach(function(current, index, arr) {
    arrNums.push(Number(current));
  });
  numbers = Number(strNums);

  return arrNums;
}

let numArr = stringsToNums(strNums); 
// // expect an array of the digits as numbers

console.log(stringsToNums(strNums));
...
const weatherStates = function(arr, key) {
  const output = [];
...
console.log(weatherStates(weather, 'weather_state_name'));

// find the id of the object in weather that has a min_temp of 15.915

const idealTemp = function(arr, minimum){
  // for (let i = 0; i < arr.length; i++) {
  // if (arr[i].min_temp === '15.915') {
    return arr.find(item => item.min_temp === minimum).id;
  };

console.log(idealTemp(weather, 15.915));