...
const displayPost = () => {
  ...
  arrayOfPosts.map((post, index) => {
    ...
    li.appendChild(text)            //why is the # sign here???
    allPosts.append(li)
  })
};
...
const comments = () => {
  const commentsVar2 = document.getElementById('comments')
  //logic to display comments
  const li= document.createElement('li')
  const text = document.createTextNode(`...`)
  li.appendChild(text)            
  commentsVar2.append(li)

}

const users = () => {
  const usersVar2 = document.getElementById('users') 
